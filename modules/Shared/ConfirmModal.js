import React from 'react'
import moment from 'moment'

export default React.createClass({
  handleConfirmClick () {
    this.props.confirmAction(this.props.trip.tripId)
  },

  render () {
    return (
      <div className="modal-background">
        <div className="modal-container modal-alert">
          <div className="modal-close" onClick={this.props.closeModal}>
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
              <polygon points="19.82 5.59 18.41 4.18 12 10.59 5.59 4.18 4.18 5.59 10.59 12 4.18 18.41 5.59 19.82 12 13.41 18.41 19.82 19.82 18.41 13.41 12 19.82 5.59"></polygon>
            </svg>
          </div>
          <div className="modal-header">
            <span>Are you sure you want to delete this outfit?</span>
          </div>
          <div className="modal-body modal-body-with-header">
            <p>City: {this.props.trip.city}</p>
            <p>Start: {moment(this.props.trip.startDate).format('dddd, MMMM Do')}</p>
            <p>End: {moment(this.props.trip.endDate).format('dddd, MMMM Do')}</p>
          </div>
          <div className="modal-actions">
            <button
              onClick={this.props.closeModal}
              data-test-id="reset-modal-cancel-button"
              className="button red cancel-modal"
            >Cancel
            </button>
            <button
              onClick={this.handleConfirmClick}
              data-test-id="reset-modal-confirm-button"
              className="button"
            >Delete
            </button>
          </div>
        </div>
      </div>
    )
  }
})